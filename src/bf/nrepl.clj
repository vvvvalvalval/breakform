...
(defn add-letsc
  ...
  (if (some? ep-id)
    (let [forms (read-tl-forms s)]
      (->> forms
        (map (fn [s1]
               (str "(bf.code/letsc " (pr-str ep-id) " " s1 ")")))
        (apply str)))
    s))
...
(defn wrap-handle
  ...
  (apply handler #_args (rewrite-args @current-id args)))

(comment
  (reset! current-id nil)

  (rewrite-args @current-id
...
(defn wrap-bf
  [handler]
  (fn [& args]
    (wrap-handle handler (vec args))))

(nrm/set-descriptor! #'wrap-bf
...
(defn in-ep
  "'In Execution Point' - Sets the current execution point id to epid.
  Subsequent commands sent to the REPL via 'eval' or 'load-file'
  will be transformed so that each form is wrapped with (bf.code/letsc epid ...)"
  [epid]
  (reset! current-id epid))
